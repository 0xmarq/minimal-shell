...
public:
  vector<string>
  tokenize(const string &input) // handles quotes, backslashes and escapes.
  {
...
public:
  ...
  char **vec_to_cstr_array( // What this do you may ask, i'll explain it.
  ...
  void executePipeline(const vector<Command> &commands)
  {
    ...
    for (int i = 0; i < n; ++i)
    {
      ...
      if (pids[i] == 0)
      {
        ...
        args.insert(args.end(), commands[i].args.begin(),
                    commands[i].args.end());
