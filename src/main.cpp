...
string find_path(string cmd){
        ...
        string path;//to put each dir path

        while(!ss.eof()){

                //taking a single dir path and we can take a single path
                //from the string as each path is separated by :
                getline(ss,path,':');
...
vector<string> split(string s){
    ...
    for(int i=start;i<s.size();i++){
        if(s[i]==' ' && sq==0 && dq==0 && pres==0){
            ...
            if(sq==0 && dq==0 && pres==0)sq=1;
            else if(sq==1 && pres==0) sq=0;
            else {
                if(pres==1 && dq==1)a+='\\';
                a+=s[i];
                pres=0;
            ...
            if(sq==0 && dq==0 && pres==0)dq=1;
            else if(dq==1 && pres==0)dq=0;
            else {
                a+=s[i];
                pres=0;
            }
...
void autocomplete(string &input,int &found,Trie &trie,int &tab){
        ...
        if(matches.size()==0){
                ...
                for(int i=0;i<matches.size();i++){
                        if(i%5==4) cout<<matches[i]<<"\n"<<flush;
                        else cout<<matches[i]<<"  "<<flush;
                }
                // cout<<"\ntoo many matches\n"<<flush;
                cout<<"\n\r\a$ "<<input<<flush;
        }
...
int execute(string input){
        ...
        while(i<parsed.size()){
                ...
                if(parsed[i]==">" || parsed[i]=="1>" || parsed[i]=="2>" ||
                        ...
                        redirection=1;
                        if(parsed[i][0]=='2')error_redirection=1;
                        if(parsed[i]=="2>>" || parsed[i]=="1>>" || parsed[i]==">>")append=1;
        ...
        //echo cmd to print text after it
                ...
                if(redirection){
                        ...
                        if(!append)fd=open(output[0].c_str(),O_WRONLY | O_CREAT | O_TRUNC, 0644);
                        else fd=open(output[0].c_str(),O_WRONLY | O_CREAT | O_APPEND, 0664 );
                        if(fd<0){
                                perror("open failed");
                                return 1;
                        }
                        ...
                        if(error_redirection) dup2(fd,STDERR_FILENO);
        ...
        // std::cout<<input<<": command not found" << std::endl;
                ...
                if(path=="")cout<<cmd<<": command not found" << endl;
                        ...
                        if(pid==-1){
                                ...
                                for(int i=0;i<executable.size();i++)ex[i]=executable[i];
        ...
        while(1){
                ...
                for(int i=0;i<cmds.size();i++){
                        ...
                        for(int j=0;j<cmds[i].size();j++){
                                ...
                                if(cmds[i][j]=='|'){
                                        if(!s.empty())pipe_split.push_back(s);
                                        s="";
                                }
                                else {
                                        s+=cmds[i][j];
                        ...
                        if(pipe_split.size()<=1){
                                ...
                                for(int i=0;i<n;i++) waitpid(pids[i],nullptr,0);
